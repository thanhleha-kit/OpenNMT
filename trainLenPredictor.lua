...
local cmd = onmt.utils.ExtendedCmdLine.new('trainLenPredictor.lua')
...
local function main()
  ...
  if checkpoint.models then
	_G.model = _modelClass.load(opt, checkpoint.models, dataset.dicts, false)
  else
	local verbose = true
  ...
  local trainer = onmt.train.LengthTrainer.new(opt)
