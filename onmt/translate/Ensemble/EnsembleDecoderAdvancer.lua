...
function DecoderAdvancer:__init(decoders, batch, contexts, max_sent_length, max_num_unks, decStates, dicts, word_pen, ensembleOps)
  ...
  self.sourceMask = onmt.utils.Cuda.convert(batch.sourceMask)
...
function DecoderAdvancer:initBeam()
  ...
  -- Define state to be { {decoder states}, {source mask}, {decoder output}, {decoder coverage}, {context},
  -- {attentions}, features, sourceSizes, step }.
  local decOutputs = nil
  local state = { self.decStates, self.sourceMask, nil, self.contexts, nil, features, sourceSizes, 1 }
...
function DecoderAdvancer:update(beam)
  ...
  local decStates, sourceMask, decOutputs, contexts, _, features, sourceSizes, t
  ...
  for i = 1, self.nModels do
	...
	decOutput, decState = self.decoders[i]:forwardOne(inputs, sourceMask, decState, context, decOutput)
  ...
  t = t + 1
  local nextState = {newStates, sourceMask, newOuts, contexts, attnOuts, nil, sourceSizes, t}
