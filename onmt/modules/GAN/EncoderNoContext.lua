...
--]]
local Encoder, parent = torch.class('onmt.EncoderNoContext', 'onmt.Sequencer')
...
function Encoder:forward(batch)
  ...
  -- final States are the RNN states at the last time step
  if finalStates == nil then
    finalStates = states
  end
  
  -- we only need the last hidden layer
  local finalState = finalStates[#finalStates]
...
--]]
function Encoder:backward(batch, gradStateOutput)
  ...
  gradStatesInput[#gradStatesInput]:copy(gradStateOutput) -- copy the gradients of the last hidden layer
