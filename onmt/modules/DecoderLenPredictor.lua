...
--]]
local DecoderLenPredictor, parent = torch.class('onmt.DecoderLenPredictor', 'onmt.Sequencer')
...
--]]
function DecoderLenPredictor:__init(inputNetwork, rnn, generator, attention, inputFeed)
  self.rnn = rnn
  ...
  self.lenGenerator = nn.Sequential()
  self.lenGenerator:add(nn.Transpose({1, 2})) -- first transpose the context matrix from BxTxH -> TxBxH
  self.lenGenerator:add(nn.SplitTable(1)) -- split the tensor into a table of tensors. Each entry is batch * hidden 
  self.lenGenerator:add(nn.ConcatTable() 
						  :add(nn.SelectTable(1)) -- Select the first time step
						  :add(nn.SelectTable(-1))) -- select the last time step
  self.lenGenerator:add(nn.JoinTable(2)) -- Concatenate them together
  self.lenGenerator:add(nn.Dropout(self.rnn.dropout))
  self.lenGenerator:add(nn.Linear(self.args.rnnSize * 2, self.args.rnnSize)) -- Predict the size 
  self.lenGenerator:add(nn.Linear(self.args.rnnSize, onmt.Constants.MAX_TARGET_LENGTH)) -- Predict the size 
  self.lenGenerator:add(nn.LogSoftMax())
  ...
  self:add(self.lenGenerator)

  self:resetPreallocation()
end

--[[ Return a new Decoder using the serialized data `pretrained`. ]]
function DecoderLenPredictor.load(pretrained)
...
function DecoderLenPredictor:serialize()
  return {
...
function DecoderLenPredictor:resetPreallocation()
  ...
  self.lenCriterion = nn.ClassNLLCriterion()
end
...
--]]
function DecoderLenPredictor:_buildModel()
  local inputs = {}
  local states = {}
...
--]]
function DecoderLenPredictor:maskPadding(sourceSizes, sourceLength)
...
Returns:
 ...
 2. `states` - All states.
--]]
function DecoderLenPredictor:forwardOne(input, prevStates, context, prevOut, t)
  local inputs = {}
...
function DecoderLenPredictor:forwardAndApply(batch, encoderStates, context, func)
...
--]]
function DecoderLenPredictor:forward(batch, encoderStates, context)
...
Parameters:
  ...
  * `criterion` - a single target criterion object
  ...
  -- ]]
function DecoderLenPredictor:backward(batch, context, outputs, criterion)
  ...
  --~ -- Before taking the loss from the decoder output, we predict the possible length from the encoder side. 
  
  local lenPred = self.lenGenerator:forward(context)
  local lenLoss = self.lenCriterion:forward(lenPred, batch.targetSize)
  local decLenOut = self.lenCriterion:backward(lenPred, batch.targetSize)
  local gradContextLen = self.lenGenerator:backward(context, decLenOut)
  
  -- Add the gradients to the context 
  gradContextInput:add(gradContextLen)
...
--]]
function DecoderLenPredictor:computeLoss(batch, encoderStates, context, criterion)
  encoderStates = encoderStates
    or onmt.utils.Tensor.initTensorTable(self.args.numEffectiveLayers,
                                         onmt.utils.Cuda.convert(torch.Tensor()),
                                         { batch.size, self.args.rnnSize })
...
--]]
function DecoderLenPredictor:computeScore(batch, encoderStates, context)
  encoderStates = encoderStates
    or onmt.utils.Tensor.initTensorTable(self.args.numEffectiveLayers,
                                         onmt.utils.Cuda.convert(torch.Tensor()),
                                         { batch.size, self.args.rnnSize })
...
function DecoderLenPredictor:getLengthPrediction(batch, context)
...
function DecoderLenPredictor:computeLenLoss(batch, context)
  
  local lenPred = self.lenGenerator:forward(context)
  local lenLoss = self.lenCriterion:forward(lenPred, batch.targetSize)
  return lenLoss * batch.size
end
